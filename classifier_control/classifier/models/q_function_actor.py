...
from classifier_control.classifier.utils.q_network import QNetwork, AugQNetwork, ActorNetwork


class QFunctionActor(BaseModel):
    ...
    def loss(self, model_output):
        ...
        qs = []
        with torch.no_grad():
            actions = self.actor_network(image_pairs)
            targetq = self.target_qnetwork(image_pairs, actions).detach()
            qs.append(targetq)
        qs = torch.stack(qs)
        ...
        losses.critic_loss = F.mse_loss(target, model_output.squeeze())
        losses.actor_loss = -self.qnetwork(image_pairs, actions).mean()
        losses.total_loss = losses.critic_loss + losses.actor_loss
...
class QFunctionActorTestTime(QFunctionActor):
    def __init__(self, overrideparams, logger=None):
        super(QFunctionActorTestTime, self).__init__(overrideparams, logger)
